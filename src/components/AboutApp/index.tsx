                ...
                {arrAuthen.map((item, index) => {
                  return (
                    <li key={index} className="flex gap-2 w-full items-center">
                      <Image width={40} height={40} alt="app" src={item.src} className=" !w-[40px] !h-[40px]"/>
                      <span className="text-lg font-normal leading-5 text-start ">{item.name}</span>
                    </li>
                  );
                })}
              </ul>
