...
export default function MainBoard() {
  ...
  useInterval(() => {
    getInfluencer().then((data) => { setData(data) }).catch((error) => { console.log(error); });
  }, 5 * 60 * 1000)
  document.getElementById("divSlider")?.offsetWidth

  useEffect(() => {
    fetch(`${process.env.SERVER}/dashboard`).then(res =>
      res.json()).then((data) => {
        setDataTop(data?.data)
      }).catch(err => {
        console.log(err);
      })
    fetch(`${process.env.SERVER}/tweet`).then(res => res.json().then((data) => {
      setTweet(data[0])
  ...
  if (width < 1024) {
    return <DashboardMobile tweet={tweet?.data} influencer={data?.data} trending={dataTop} />
  }
    ...
    <LayoutDashBoard>
      ...
      <div className="grid md:grid-cols-2 gap-3 grid-cols-1 mt-3">
        <div className=" rounded-md h-[800px]">
          <div id="divSlider" className="px-4 py-4 rounded-md h-[300px] mb-3 bg-[#fff] drop-shadow-xl">
            <Slider {...settings} className="w-full h-[280px] rounded-md custom-slider">
          ...
          <div className="flex justify-between gap-2 2xl:gap-3">
            {tweet?.data?.map((item: any, index: any) => {
              ...
              return (
                <Link href={item?.link ? item?.link : 'https://twitter.com/BitscopeAI'} key={index} className="w-[33%] drop-shadow-md" target="_blank">
                  <div className="flex bg-[#fff] hover:bg-[#F0F0F0] py-4 flex-col gap-2 2xl:gap-4 rounded-md h-[460px]">
                    <div className="flex px-3">
                      ...
                      <div>
                        <div className="flex">
                          ...
                          <div className="ml-1 mt-[2px]"><IconInfulencerSVG /></div>
                        </div>
                        <div className="flex">
                    ...
                    <div className="break-words px-3">
                      <div className="font-Inter text-sm font-normal h-[220px] 2xl:h-[180px] 3xl:h-[150px] overflow-hidden">{render.map((line: any, index: any) => {
                        return (
                          <p key={index}>
                            {line}
                          </p>
                    ...
                    {item?.images && <div className="rounded-md px-3">
                      <Image width={174} height={100} src={item?.images[0]} alt="" className="rounded-md h-[120px] w-full 2xl:h-[130px] 3xl:h-[180px]" />
                    </div>}
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="w-3/4 mt-1">
                        <div className="flex">
                          ...
                          {item?.data?.isActive ? (item?.data?.isActive == 'blue' ? <div className="mr-1 mt-[3px]"><IconInfulencerSVG /></div> : <div className="mr-1 mt-[3px]"><IconYellowSVG /></div>) : <div></div>}
                          <div className="mb-2">
                            <p className="font-Inter font-normal text-base text-[#585858] mr-1">{'@' + item?.data?.userId}</p>
                          </div>
                        ...
                        <div className="pb-2">
                          <p className="text-start font-Inter text-sm font-normal">{item?.data?.content.slice(0, 100) + '... '}
                            <Link
                              href={item?.data?.link || "https://twitter.com/BitscopeAI"}
                              target="_blank"
                        ...
                        {item?.data?.video &&
                          <video controls width="100%" height="100%" src={item?.data?.video?.link} className="rounded-md max-h-[267px]"></video>
                        }
                        {item?.data?.images && item?.data?.images?.length !== 0 &&
                          <div className={`grid ${item?.data?.images?.length > 1 ? ' grid-col-2 grid-flow-col gap-1 ' : 'grid-col-1 grid-flow-col'}`}>
                                    ...
                                    <img
                                      ...
                                      className={`${item?.data?.images?.length > 1 ?
                                        item?.data?.images?.length > 2 ?
                                          ((index === 1 || index === 2) ?
                                            'h-[133px]' : 'h-[267px]') : 'w-full h-[267px]' : 'w-auto max-h-[267px]'} rounded-md`}
                                    />
                                  </picture>
                                </div>
                              )
